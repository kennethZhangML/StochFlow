...
class StochasticProcess:
    ...
    def evolve_density(self, density, time_step):
        new_density = (density.get_density() - time_step *
                       np.gradient(self.velocity_field * density.get_density()))
        new_density += time_step * np.gradient(
            self.forward_drift * density.get_density()) + self.diffusivity * np.gradient(
            np.gradient(density.get_density()))
...
print(f"Backward Samples: {backward_samples}")
print(f"Forward Samples: {forward_samples}")
